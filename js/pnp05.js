...
//	$("input[type='radio']").checkboxradio();
	$("#locationType").selectmenu({
		select: function( event, ui ) {
			if(ui.item.label=='Boat'){
				type = 'boat';
				getPlaces(type);
	...
	// detect when weather tab is clicked on - make weather load only when tab is called
...
//		$('#map2_buttons').append('<button type="button" id="deletePlace" 	class="ui-button">delete</button>');
		...
		$('#option2').html('<a href="#" id="logout" onclick="location.href='+logoutLink+'">logout '+username+'</a><br>');
	...
	{
		$('#option2').html('<a href="#" id="login" >login</a><br>');
